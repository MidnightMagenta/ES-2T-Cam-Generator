...
namespace qrk {
    inline qrk::vec2u GetPrimaryScreenSize() {
        unsigned int sizeX = GetSystemMetrics(SM_CXSCREEN);
        unsigned int sizeY = GetSystemMetrics(SM_CYSCREEN);

        return qrk::vec2u({sizeX, sizeY});
    }
